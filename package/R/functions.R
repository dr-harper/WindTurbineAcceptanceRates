...
LogOddsPlotGraph <- function(OddsTable, PlotTitle, Sort = FALSE){
  ...
  plotlogodds <- ggplot(ModelDF, aes(x = term, y = odds - offset, fill = Relationship)) +
    ...
    scale_x_discrete(limits = rev(Terms), labels = function(x) str_wrap(x, width = 10)) +
...
#'
#' @param cache the filepath of the cached file
#' @param analysis the filepath of the analysis file
#'
#' @export
#'
runAnalysis <- function(cache = "../_code/outputs/codeOutputs.RData", analysis = "AnalysisComplete.Rmd"){

  # Run the full analysis if the cache does not exist
  if(!file.exists(cache)){

    message("Analysis Results not yet run. Running File code/AnalysisComplete.Rmd")
    setwd("../_code/")
    bookdown::render_book(analysis)
    message("Analysis Outputs saved in cache")

  }

  # Load the cached results
  message("Analysis results loaded from cache for use within report")
  load("../_code/outputs/codeOutputs.RData", envir = .GlobalEnv)
