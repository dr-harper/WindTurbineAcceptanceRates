...
turbines_numeric <- turbines_numeric %>%
  dplyr::select(-one_of(c('RO Banding', 'NA', 'NA.1', 'NA.2', 'NA.3', 'NA.4',
                   'NA.5', 'NA.6', 'Y Coordinate', 'X Coordinate'))) %>%
  magrittr::set_rownames(NULL) # Delete RO banding category
...
df_turbine_trunc <- 
  df_turbine %>%
  dplyr::select(Airports, AONB, ARoads, BRoads, HCoast, HVpowerline, MilitarySites, MinRoads, Motorways, NationalParks, NNR, Powerlines, PrimaryRoads, Railway, RAMSAR, SACS, SPA, SSSI, UrbanLarge, UrbanSmall, UrbanRegions)
...
variables_pars <- variables_pars$term
...
```{r}
# Make a segmented table
england <- WindAnalysis::LogisticResultsTable(segmented_country$models$England) %>%
  dplyr::select(c('Variable' ,'Odds Ratio', 'Sig.'))
scotland <- WindAnalysis::LogisticResultsTable(segmented_country$models$Scotland) %>%
  dplyr::select(c('Odds Ratio', 'Sig.'))
wales <- WindAnalysis::LogisticResultsTable(segmented_country$models$Wales) %>%
  dplyr::select(c('Odds Ratio', 'Sig.'))

segmented <- bind_cols(england, scotland, wales) %>%
  set_names(c("Variable", rep(c("OR", ""),3)))

segmented$Variable <- matchNames(segmented$Variable)
...
save(segmented, segmented_country, summary_split_model, model_pars, wind_models,
...
# Reponse to Peer Review

Several potential alterations were suggested within the peer review process. This proces

## Proposed Model Alterations

- Plateau the year variable to 2006: 

```{r}

# Truncate any years later than 2006
df_turbine$year[df_turbine$year > 2006] <- 2006

# Rerun model 7
wind_models$No7 <- LogisticModel(variables_model_7, df_turbine)


summary(wind_models$No7)
LogisticDiagnostics(wind_models$No7)
```

- Does splitting the model into two (before/after 2006) help improve model fit?

```{r YearNested, include=FALSE}
variables_pars <- variables_pars[variables_pars != "SNP_PC_sha"]
df_turbine$yearCategorySplit <- ifelse(df_turbine$year>2006, "After", "Before")

# Uses a range of custom functions within "Segmented_LogisticModelsComplete" to split data
segmented_year <- Segmented_LogisticModelsComplete(df_turbine, "yearCategorySplit", variables_pars, "Status.Summary", 0.2, 1)

# Build Optimised Parameter Models
models_split <- NULL
models_split$before <- FindBestModel(df = segmented_year$data$`Before`, variables_pars)
models_split$after <- FindBestModel(df = segmented_year$data$`After`, variables_pars, steps = 5)

# Summarise the results
summary_split_model <- LogResultsTable(segmented_year$models$`After`,
                                       segmented_year$models$`Before`) %>%
  set_colnames(c("2010 Onwards", "Before 2010"))
```

## Turbine Size 

- How does turbine capacity and project size correlate?

```{r}
df_turbine_capacity <- df_turbine %>%
  dplyr::filter(Turbine.Capacity..MW. != 0)

ggplot(df_turbine_capacity, aes(Turbine.Capacity..MW., Capacity)) +
  geom_point()
```


```{r}
cor(df_turbine$Turbine.Capacity..MW., df_turbine$Capacity, use = "complete.obs")
```
